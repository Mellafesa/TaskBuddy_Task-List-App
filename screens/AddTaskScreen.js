...
import { View, TextInput, StyleSheet, Platform, FlatList, TouchableOpacity, Text, Alert } from 'react-native';
...
export default function AddTaskScreen({ route, navigation }) {
  ...
  const handleAddTask = () => {
    ...
    .then(() => {
      ...
      addTask({id: newTaskRef.key, ...newTask});
      navigation.goBack();
    })
    .catch((error) => {
      console.error('Terjadi kesalahan saat menambahkan task: ', error);
      Alert.alert('Kesalahan', 'Tidak dapat menambahkan task. Silakan coba lagi.');
    });
